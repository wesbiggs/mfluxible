...
| `image_strength` | float or null | `0.4` with `image`; `0.0` with `image` **and** `mask` | how strongly `image` constrains the output, `0.0`–`1.0`; only meaningful, and only accepted, alongside `image` — a **400** if set without it. The masked default differs because the field governs something else there; see [Inpainting a region](#inpainting-a-region) |
...
**`image_strength` defaults to `0.0` here rather than `0.4`, and leaving it there is almost always right.** This is the one place mflux's inverted convention bites hardest, so the default follows the mask rather than the field: with a mask, `image_strength` no longer governs how much of the *frame* survives — the mask does that — it governs only how much of the original content *inside* the mask survives. At `0.4` the old object comes back very nearly intact in the region you just painted, which reads as the mask having been ignored — measured on a 768×768 Z-Image-Turbo run with identical seed and mask, `0.4` moved the masked region by 5.1/255 against 41.6 at `0.0`, after a full generation that reported success either way. `0.0` starts the masked region from sigma 0 — pure noise — and is what actually replaces content.

An explicit value still wins, and the useful range above `0.0` is narrower and more interesting than "restyle": it is a **composition anchor**. At `0.0` the region is reinvented from noise, so where the new content lands inside the box is decided by the prompt alone. One rung in — `0.1`–`0.2` at 9 steps, both of which floor to `init_time_step` 1 — the region starts from the *encoded original* rather than from noise, so the new content inherits the old one's placement, scale and outline. That is the setting to reach for when the prompt cannot reproduce the original composition, which is the normal case for a **photograph**: "a man reading a book" will not be composed with the same pose and framing as the photo you started from, and anchoring is the only thing that keeps the replacement where the original was.

The cost is bleed-through, and it scales with the anchor. Replacing an "S" chest emblem with an "F" on the [inpainting example](#inpainting-a-region) above: at `0.0` the badge is a clean F at a size the prompt chose; at `0.2` with [`fractional_start`](#fractional-start) it keeps the original shield's exact position and outline and the old S contaminates the letterform. So anchor when the new content *should* share the old one's geometry, and don't when it shouldn't. There is no setting that does both. Because the useful range is narrow and still quantized to `1/steps`, [`fractional_start`](#fractional-start) is worth more here than it is for plain image-to-image.
...
A bad body is a 400 before the stream starts, the same as a generation. The response is
...
the one a mask is validated against. Compare them to your own oriented dimensions and
discard the regions on a mismatch; that catches this page and the server disagreeing
about the same image's orientation.

It does **not** catch the likelier problem, which is local: a client that swaps the
image it is displaying while a detection is still open. The stream outlives the image
that started it, and a replacement of identical size looks the same on this field. Tag
the displayed image with a token of your own, capture it when you submit and check it
when regions arrive — the harness keeps a counter. Nothing needs to travel *inside* the
image for this: the server pairs each job with the exact bytes it stashed and hands the
worker that file unchanged, so the identity in question is only ever which image the
client is showing.

**One detection at a time.** A second `detect` supersedes the first rather than queueing
behind it, and the superseded stream is closed with an `error` naming the reason — which
...
```json
{"job_id": "75d3…", "image_path": "/…/75d3….png", "width": 768, "height": 768}
```
...
```json
{"regions": [{"label": "red apple", "box": [0.305, 0.344, 0.712, 0.736]}], "width": 768, "height": 768}
```

Or `{"error": "…"}` instead of `regions` — a worker that could not run is a normal
outcome the waiting client should see, not a transport failure, so the message is
carried in the body rather than signalled with a status code.

Replies `{"delivered": true}`, or `{"delivered": false, "reason": …}` with a 200 when
that job is no longer the pending one. That is not an error worth a 4xx: the usual cause
is the user starting a new detection while this one was still running.
