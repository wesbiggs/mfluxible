...
That file is why `requirements-dev.txt` pulls in `clients/requirements-mcp.txt` by reference: the dev environment is a superset of every component, which is the opposite of the separation the four requirements files keep for *installs* (see `CLAUDE.md`) — an MCP-only install still never sees mflux. It does **not** pull in `requests`, so `clients/stream_client.py` is still not importable on the runner and nothing covers it.
